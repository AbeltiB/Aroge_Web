...
export function StatCard({
  ...
  const content = (
    <Card hoverable className="group relative overflow-hidden border-transparent hover:border-canvas-300 dark:hover:border-dark-line">
      <span className={`absolute top-0 left-0 right-0 h-[3px] ${ACCENT_BAR[accent]}`} />
      <div className="p-5 pt-[18px]">
        <div className="flex items-center gap-1.5 text-ink-400 dark:text-dark-text-soft">
          <Icon size={13} />
          <p className="text-xs font-semibold uppercase tracking-wide">{label}</p>
        </div>
        <p className="text-3xl font-display font-bold mt-2 tracking-tight text-ink-900 dark:text-dark-text tabular-nums">{value}</p>
        {sub && <p className="text-xs mt-1.5 text-ink-400 dark:text-dark-text-soft">{sub}</p>}
        {href && (
